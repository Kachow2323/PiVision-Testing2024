...
public final class Main {
  ...
  public static void main(String... args) {
    ...
    // loop forever, communicating with the network every 30ms
    while(true) {
      try {
        communicationThread.periodic();
        // This thread should run periodic every 30 ms, so we sleep until the next tick
        Thread.sleep(30);
      } catch (InterruptedException ex) {
        System.err.println("Encountered error during execution" + ex.getLocalizedMessage());
        return;
      }
    }
  }
